...
def classifier_wrapper(classifier, classifier_type, test_sample):
    """
    A wrapper to pattern classification.
    ARGUMENTS:
    classifier: a classifier object of type sklearn.svm.SVC or kNN (defined in this library) or
        sklearn.ensemble.RandomForestClassifier or
        sklearn.ensemble.GradientBoostingClassifier  or
...
def extract_features_and_train(paths, mid_window, mid_step, short_window,
    ...
    print("\nSelected params: {0:.5f}".format(best_param))
...
def feature_extraction_train_regression(path_list, mid_window, mid_step,
    ...
    ARGUMENTS:
        path_list:        path of directory containing the WAV files
                         and Regression CSVs
    ...
    features, _, filenames = \
        aF.multiple_directory_feature_extraction(path_list, mid_window,
    ...
    for path in path_list:
        pathname = path + os.sep + "*.csv"
        csv_files += glob.glob(pathname)
    if not csv_files:
        print("Error: Regression CSVs were not found in sound directiories.")
    elif len(csv_files) < len(path_list):
        print("Error: Not all of Regression CSVs were found in sound directories.")
    regression_labels = []
    regression_names = []
    f_final = []
    for csv_file_path in csv_files:
        print("Found CSV file: " + csv_file_path)
        ...
        regression_names.append(ntpath.basename(csv_file_path).replace(".csv", ""))
        if len(features) == 0:
    ...
    for iRegression, r in enumerate(regression_names):
        ...
        bestParam, error, berror = evaluate_regression(
            features=f_final[iRegression], labels=regression_labels[iRegression],
            n_exp=100, method_name=model_type, params=model_params)
        errors.append(error)
        ...
        if model_type == "svm" or model_type == "svm_rbf" \
            ...
            print("3:" + model_name)
            with open(model_name + "_" + r, 'wb') as file_id:
                cPickle.dump(classifier, file_id)
...
def evaluate_classifier(features, class_names, n_exp, classifier_name, params,
    ...
    for Ci, C in enumerate(params):
        ...
        for e in range(n_exp):
            ...
            print("Param = {0:.5f} - classifier Evaluation "
                  "Experiment {1:d} of {2:d}".format(C, e + 1, n_exp), end="\r")
            # split features:
            ...
            cm = cm + cmt
        print("{}({:.3f}) - classifier Evaluation Experiment {:d} of {:d}{}"
              .format(classifier_name, C, e + 1, n_exp, 50 * " "))
        np.math.factorial(1000)
        cm = cm + 0.0000000010
    ...
    for i in range(len(pre_class_all)):
        print("\t{0:4.3f}".format(params[i]), end="")
        for c in range(len(pre_class_all[i])):
            print("\t{0:4.1f}\t{1:4.1f}\t{2:4.1f}"
                  .format(100.0 * pre_class_all[i][c],
                          100.0 * rec_classes_all[i][c],
                          100.0 * f1_classes_all[i][c]),
                  end="")
        print("\t{0:4.1f}\t{1:4.1f}".format(100.0 * ac_all[i], 100.0 * f1_all[i]),
              end="")
        if i == best_f1_ind:
            print("\t best f1", end="")
        if i == best_ac_ind:
...
def print_confusion_matrix(cm, class_names):
    ...
    if cm.shape[0] != len(class_names):
        print("print_confusion_matrix: Wrong argument sizes\n")
        return
    # print("\t", end="")
    for c in class_names:
        if '/' in c:
    ...
    for i, c in enumerate(class_names):
        ...
        for j in range(len(class_names)):
            print("\t{0:4.2f}".format(100.0 * cm[i][j] / np.sum(cm)), end="")
        print("")
